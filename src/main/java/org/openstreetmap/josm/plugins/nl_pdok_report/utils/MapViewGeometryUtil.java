...
/**
 * Utility class to convert entities like {@link Bounds} and {@link FeedbackSequence} into {@link Shape}s that can then
 * easily be drawn on a {@link MapView}s {@link Graphics2D}-context.
 */
public final class MapViewGeometryUtil {
   ...
   * 
   * @param mv
   *          the MapView that is used for the LatLon-to-Point-conversion and that determines the Bounds from which the
   *          downloaded Bounds are subtracted
   * @param downloadBounds
   *          multiple {@link Bounds} objects that represent the downloaded area
